...
output "bandwidth_type_filter_validation" {
  value = alltrue([for v in local.test_results.share_bandwidth_types[*].bandwidth_type : v == local.test_refer.bandwidth_type])
}

output "name_en_filter_validation" {
  value = alltrue([for v in local.test_results.share_bandwidth_types[*].name_en : v == local.test_refer.name_en])
}

output "name_zh_filter_validation" {
  value = alltrue([for v in local.test_results.share_bandwidth_types[*].name_zh : v == local.test_refer.name_zh])
}

output "public_border_group_filter_validation" {
  value = alltrue([for v in local.test_results.share_bandwidth_types[*].public_border_group : v == local.test_refer.public_border_group])
}
`
